...
TYPE *find_date(int day_year, int year)
{
    ...
    *date = (TYPE) day_year;
    ...
    febLDay = getFebL(year) + janLDay;
    marLDay = febLDay + 31;
    aprLDay = marLDay + 30;
    mayLDay = aprLDay + 31;
    juneLDay = mayLDay + 30;
    julyLDay = juneLDay + 31;
    augLDay = julyLDay + 31;
    sepLDay = augLDay + 30;
    octLDay = sepLDay + 31;
    novLDay = octLDay + 30;
    ...
    if (*date <= janLDay)
        *(date + 1) = jan;
    else if (*date <= febLDay)
    {
        *date -= janLDay;
        *(date + 1) = feb;
    }
    else if (*date <= marLDay)
    {
        *date -= febLDay;
        *(date + 1) = mar;
    }
    else if (*date <= aprLDay)
    {
        *date -= marLDay;
        *(date + 1) = apr;
    }
    else if (*date <= mayLDay)
    {
        *date -= aprLDay;
        *(date + 1) = may;
    }
    else if (*date <= juneLDay)
    {
        *date -= mayLDay;
        *(date + 1) = june;
    }
    else if (*date <= julyLDay)
    {
        *date -= juneLDay;
        *(date + 1) = july;
    }
    else if (*date <= augLDay)
    {
        *date -= julyLDay;
        *(date + 1) = aug;
    }
    else if (*date <= sepLDay)
    {
        *date -= augLDay;
        *(date + 1) = sep;
    }
    else if (*date <= octLDay)
    {
        *date -= sepLDay;
        *(date + 1) = oct;
    }
    else if (*date <= novLDay)
    {
        *date -= octLDay;
        *(date + 1) = nov;
    }
    else if (*date <= decLDay)
    {
        *date -= novLDay;
        *(date + 1) = dec;
    }
